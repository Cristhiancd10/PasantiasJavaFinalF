...
public class AgendamientoDAO {
	...
	public List<Agendamiento> listarAgendamientosV() {
		String estado="VISITA TECNICA";
		String jpql = "SELECT agd FROM Agendamiento agd  WHERE agd.estado = :a";
		...
		for (Agendamiento agendamiento : agendamientos) {
			agendamiento.getTecnicoAsigna();
			agendamiento.getRegistro().getCliente().getNombre();
			agendamiento.getRegistro().getCliente().getDireccionPrincipal();	
			//modifique esto
